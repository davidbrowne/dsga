...
* Removed offset arrays from `dsga::vec_storage` and `dsga::vec<T,S>`. The only offset array needed is for `dsga::swizzle_vec` indirection.
* Made `dsga::swizzle_vec` iterators' data members **protected**.
* `ostream_output.hxx` now includes `<iomanip>`.
* Added `CTest` support in `CMakeLists.txt`.
* Updated `std::ostream` formatters for `dsga::vec_interface` and its derived classes so we don't change the state of the stream beyond outputting the `dsga` class instances.
* Fixed `std::format` formatters for `dsga::vec_interace` and `dsga::mat` mostly due to retro-active changes to the `c++20` standard.
* Changed an exact zero comparison to a toleranced comparison in all the `dsga::mat::inverse()` specializations.
* Update `vec_like` concept. Created new `writable_vec_like` concept. Created new `vec_dimension` concept for vector dimension. Created new `mat_dimension` concept for matrix row and column sizes. Created new `vec_scalar` concept for vector element type. Created new `vec_size` concept for vector size.
* Deployed new concepts to namespace machinery functions, and to `dsga::vec_interface` and its derived classes, and to `dsga::mat` and its derived classes.
