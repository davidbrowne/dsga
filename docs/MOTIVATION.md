...
I wanted to expand the point/vector class that we use at work. We have x, y, and z data members, and accessors functions X(), Y(), and Z(), but that is the only way to get at the data. I wanted something more flexible that would use contiguous memory like an array, but still be useful with x, y, z public data member access. I do *NOT* want data function access and public member access as the only ways to get x, y, and z data. I wanted array-like access.

I stumbled across a blog article about this very type of issue, and once I better understood anonymous unions and the common initial sequence concept (since verified with ```std::is_corresponding_member<>```), I was ready to write this library. I ended up writing it for my own projects, as the stuff at work is too entrenched to change given our resources.
