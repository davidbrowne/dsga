...
namespace detail
{
	...
	constexpr T quadratic_recursive_bezier_ordinate_eval(const dsga::vector_base<W, T, 3, D> &control_points, T t) noexcept
	{
...
requires (C > 1)
constexpr dsga::basic_vector<T, C> quadratic_recursive_bezier_eval(const dsga::vector_base<W1, T, C, D1> &p0,
																   ...
																   T t) noexcept
{
	...
	return [&]<std::size_t ...Is>(std::index_sequence<Is...>) noexcept
	{
		...
		return dsga::basic_vector(detail::quadratic_recursive_bezier_ordinate_eval(coord_matrix.row(Is), t)...);
...
constexpr T quadratic_recursive_bezier_eval(const dsga::vector_base<W1, T, 1, D1> &p0,
											...
											T t) noexcept
{
	...
	return detail::quadratic_recursive_bezier_ordinate_eval(dsga::basic_vector(p0[0], p1[0], p2[0]), t);
}
...
namespace detail
{
	...
	template <bool W, dsga::floating_point_scalar T, typename D>
	constexpr T quadratic_polynomial_bezier_ordinate_eval(const dsga::vector_base<W, T, 3, D> &control_points, T t) noexcept
	{
...
template <bool W1, dsga::floating_point_scalar T, std::size_t C, typename D1, bool W2, typename D2, bool W3, typename D3>
requires (C > 1)
constexpr dsga::basic_vector<T, C> quadratic_polynomial_bezier_eval(const dsga::vector_base<W1, T, C, D1> &p0,
																	const dsga::vector_base<W2, T, C, D2> &p1,
																	const dsga::vector_base<W3, T, C, D3> &p2,
																	T t) noexcept
{
	...
	{
		// evaluate the bezier function for each ordinate (i.e., row of control points)
		return dsga::basic_vector(detail::quadratic_polynomial_bezier_ordinate_eval(dsga::basic_vector(p0[Is], p1[Is], p2[Is]), t)...);
	}(std::make_index_sequence<C>{});
}

// same as previous function, but specializing for length 1 vector case
template <bool W1, dsga::floating_point_scalar T, typename D1, bool W2, typename D2, bool W3, typename D3>
constexpr T quadratic_polynomial_bezier_eval(const dsga::vector_base<W1, T, 1, D1> &p0,
											 const dsga::vector_base<W2, T, 1, D2> &p1,
											 const dsga::vector_base<W3, T, 1, D3> &p2,
											 T t) noexcept
{
	// since the arguments are all length 1, we can't create a matrix and do the lambda pack wrapper,
	// so we directly invoke the evaluator for the one ordinate
	return detail::quadratic_polynomial_bezier_ordinate_eval(dsga::basic_vector(p0[0], p1[0], p2[0]), t);
}
...
namespace detail
{
	// recursive interpolation approach (de Casteljau's algorithm) to show off vector functions and swizzling
	template <bool W, dsga::floating_point_scalar T, typename D>
	constexpr T cubic_recursive_bezier_ordinate_eval(const dsga::vector_base<W, T, 4, D> &control_points, T t) noexcept
	{
		// not sure of real type of control_points, so make a basic_vector so we can swizzle
...
requires (C > 1)
constexpr dsga::basic_vector<T, C> cubic_recursive_bezier_eval(const dsga::vector_base<W1, T, C, D1> &p0,
															   const dsga::vector_base<W2, T, C, D2> &p1,
															   const dsga::vector_base<W3, T, C, D3> &p2,
															   ...
															   T t) noexcept
{
	...
	{
		// evaluate the bezier function for each ordinate (i.e., row of control points)
		return dsga::basic_vector(detail::cubic_recursive_bezier_ordinate_eval(coord_matrix.row(Is), t)...);
	}(std::make_index_sequence<C>{});
}

// same as previous function, but specializing for length 1 vector case
template <bool W1, dsga::floating_point_scalar T, typename D1, bool W2, typename D2, bool W3, typename D3, bool W4, typename D4>
constexpr T cubic_recursive_bezier_eval(const dsga::vector_base<W1, T, 1, D1> &p0,
										const dsga::vector_base<W2, T, 1, D2> &p1,
										const dsga::vector_base<W3, T, 1, D3> &p2,
										...
										T t) noexcept
{
	// since the arguments are all length 1, we can't create a matrix and do the lambda pack wrapper,
	// so we directly invoke the evaluator for the one ordinate
	return detail::cubic_recursive_bezier_ordinate_eval(dsga::basic_vector(p0[0], p1[0], p2[0], p3[0]), t);
}
...
namespace detail
{
	// Bernstein basis polynomial algorithm
	template <bool W, dsga::floating_point_scalar T, typename D>
	constexpr T cubic_polynomial_bezier_ordinate_eval(const dsga::vector_base<W, T, 4, D> &control_points, T t) noexcept
	{
		auto t_complement = T(1) - t;
...
template <bool W1, dsga::floating_point_scalar T, std::size_t C, typename D1, bool W2, typename D2,
	bool W3, typename D3, bool W4, typename D4>
requires (C > 1)
constexpr dsga::basic_vector<T, C> cubic_polynomial_bezier_eval(const dsga::vector_base<W1, T, C, D1> &p0,
																const dsga::vector_base<W2, T, C, D2> &p1,
																const dsga::vector_base<W3, T, C, D3> &p2,
																const dsga::vector_base<W4, T, C, D4> &p3,
																T t) noexcept
{
	...
	{
		// evaluate the bezier function for each ordinate (i.e., row of control points)
		return dsga::basic_vector(detail::cubic_polynomial_bezier_ordinate_eval(dsga::basic_vector(p0[Is], p1[Is], p2[Is], p3[Is]), t)...);
	}(std::make_index_sequence<C>{});
}

// same as previous function, but specializing for length 1 vector case
template <bool W1, dsga::floating_point_scalar T, typename D1, bool W2, typename D2, bool W3, typename D3, bool W4, typename D4>
constexpr T cubic_polynomial_bezier_eval(const dsga::vector_base<W1, T, 1, D1> &p0,
										 const dsga::vector_base<W2, T, 1, D2> &p1,
										 const dsga::vector_base<W3, T, 1, D3> &p2,
										 const dsga::vector_base<W4, T, 1, D4> &p3,
										 T t) noexcept
{
	// since the arguments are all length 1, we can't create a matrix and do the lambda pack wrapper,
	// so we directly invoke the evaluator for the one ordinate
	return detail::cubic_polynomial_bezier_ordinate_eval(dsga::basic_vector(p0[0], p1[0], p2[0], p3[0]), t);
}
