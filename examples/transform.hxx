...
// Our definition of a transformation matrix does NOT include shearing or scaling. The rotations are
// right-handed.
// 
...
namespace dsga
{
	...
	[[nodiscard]] constexpr auto transform_inverse(const basic_matrix<T, 3, 3> &arg) noexcept -> basic_matrix<T, 3, 3>
	{
	...
	template <floating_point_scalar T>
	[[nodiscard]] constexpr auto transform_inverse(const basic_matrix<T, 4, 4> &arg) noexcept -> basic_matrix<T, 4, 4>
	{
		// transpose of rotation part of arg is the inverse of the rotation part,
	...
	[[nodiscard]] constexpr auto translation_matrix(const vector_base<W, T, 2, D> &translation) noexcept -> basic_matrix<T, 3, 3>
	{
	...
	//
	template <bool W, floating_point_scalar T, typename D>
	[[nodiscard]] constexpr auto translation_matrix(const vector_base<W, T, 3, D> &translation) noexcept -> basic_matrix<T, 4, 4>
	{
	...
	template <floating_point_scalar T>
	[[nodiscard]] constexpr auto rot_2d(T theta) noexcept -> basic_matrix<T, 3, 3>
	{
	...
	template <floating_point_scalar T>
	[[nodiscard]] constexpr auto rot_x_axis(T theta) noexcept -> basic_matrix<T, 4, 4>
	{
		auto cos_theta = cos(theta);
		auto sin_theta = sin(theta);
	...
	template <floating_point_scalar T>
	[[nodiscard]] constexpr auto rot_y_axis(T theta) noexcept -> basic_matrix<T, 4, 4>
	{
		auto cos_theta = cos(theta);
		auto sin_theta = sin(theta);
	...
	template <floating_point_scalar T>
	[[nodiscard]] constexpr auto rot_z_axis(T theta) noexcept -> basic_matrix<T, 4, 4>
	{
		auto cos_theta = cos(theta);
		auto sin_theta = sin(theta);
	...
	[[nodiscard]] constexpr auto rot_any_axis(T theta,
											  const vector_base<W1, T, 3, D1> &axis,
											  const vector_base<W2, T, 3, D2> &origin) noexcept -> basic_matrix<T, 4, 4>
	{
		auto c_t = cos(theta);
		auto s_t = sin(theta);
		auto v_t = T(1.0) - c_t;
	...
	constexpr auto renormalize_transformation_matrix(const basic_matrix<T, 4, 4> &mat, T tol) noexcept -> basic_matrix<T, 4, 4>
	{
		...
		// leave translation values alone
